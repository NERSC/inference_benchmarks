...
def getFP32(input_file, output_prefix, output, batch_size=128, workspace_size=1<<30):
...
if "__main__" in __name__:
  ...
  #do the conversion
  if f.FP32:
    getFP32(input_file=f.input_file, output_prefix=f.output_prefix, output=["generator/Tanh"], batch_size=f.batch_size, workspace_size=f.workspace_size)
  if f.FP16:
    getFP16(input_file=f.input_file, output_prefix=f.output_prefix, output=["generator/Tanh"], batch_size=f.batch_size, workspace_size=f.workspace_size)
  if f.INT8:
    ...
    dummy_input = np.random.uniform(size=(f.batch_size*f.num_calibration_runs, 64)).astype(np.float32)
    #time graph
